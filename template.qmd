...
```{r}
# reading in the data 
grazing_raw <- read_csv(here("data", "grazing_env.csv"))
hdi_raw <- read_csv(here("data", "hdi_clean.csv"))

```

What we need to do before running the PCA: 
- what we pass through has to be numeric
- need to drop NA's
...
PC1 explains 71.45% of the variation in our outcome variable, PC2 explains 9.6% of the outcome variable, 
and PC3 explains 7.8% of the outcome variable. Together, they explain 88.9% of the variation in the outcome 
variable.
...
Create a dataframe with the necessary ingredidents to make a screeplot. One piece that may not be immediately obvious is provided to get you started. We'll need to keep track of which PC is which, `colnames(hdi_pca$rotation)` will give you the order of the PCs.
...
# Screeplot
ggplot(pct_expl_df, aes(x=pc,y=pct, fill=pc)) +
  geom_col() +
...
```
Interpretation: gender inequality index and inequality in life expectancy are highly correlated 
